...
```
├── ingest/
│   └── validate_and_partition.py   # ETL ingestion script
├── datalake/
|   └── raw
|       └── events.csv
│   └── events/                     # Partitioned Parquet output (created by script)
│       └── date=YYYY-MM-DD/
│           └── category=<cat>/
│               └── *.parquet
├── scripts/
│   └── analytical_queries.sql      # PostGIS analytical queries
├── bad_records.log                 # Generated by ingestion script
├── requirements.txt
├── docker-compose.yml
└── README.md
```
...
- **Solution**: The ingestion script explicitly increases these limits using `max_partitions=2048` and `max_open_files=512`. This allows the pipeline to handle high-cardinality data (many unique dates and categories) without crashing.
...
```
2. **Execute ingestion pipeline**: Use the following command to trigger the ingestion pipeline
```bash
docker exec -it togo_ingest_app python ./ingest/validate_and_partition.py
```
3. **Verify the results**: You can check in the ingestion container
    - **Cleaned Data**: The partitioned Parquet files will be generated in the `./datalake/events/` directory.
    - **Logs**: Validation errors and "bad records" are written to `./bad_records.log`.
